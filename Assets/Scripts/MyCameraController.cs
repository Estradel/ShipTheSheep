...
public class MyCameraController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            var translation = new Vector3(-Input.GetAxis("Mouse X"), -Input.GetAxis("Mouse Y"), 0);
        ...
        else // If the mouse is near the screen border, pan in this direction
        {
            ...
            if (Input.mousePosition.x < 50)
                translation.x = -1 * 2;
            else if (Input.mousePosition.x > Screen.width - 50) translation.x = 1 * 2;
            if (Input.mousePosition.y < 50)
                translation.z = -1;
            else if (Input.mousePosition.y > Screen.height - 50) translation.z = 1;
